...
<template>
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="max-w-4xl mx-auto px-4 py-4 flex items-center gap-3">
        <div class="w-10 h-10 bg-goztepe-red rounded-full flex items-center justify-center shrink-0">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <path stroke-linecap="round" stroke-width="1.5" d="M12 2C6.5 6 6.5 18 12 22M12 2c5.5 4 5.5 16 0 20M2 12h20" />
          </svg>
        </div>
        <div>
    ...
    <main class="max-w-4xl mx-auto px-4 py-6 space-y-4">
      <!-- Controls -->
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-wrap items-end gap-4">
        <!-- Season selector -->
        <div class="flex flex-col gap-1">
          ...
          >
            <option v-for="s in store.availableSeasons" :key="s" :value="s">{{ s }}</option>
          </select>
        </div>
      ...
      <!-- Summary -->
      <div v-if="store.games.length > 0" class="flex items-center gap-2 text-xs text-gray-400 px-1">
        <span>Toplam <strong class="text-gray-600">{{ store.filteredGames.length }}</strong> maç</span>
        <span v-if="store.selectedLeagueCodes.length">
          · <strong class="text-gray-600">{{ store.selectedLeagueCodes.length }}</strong> lig seçili
        </span>
      </div>
