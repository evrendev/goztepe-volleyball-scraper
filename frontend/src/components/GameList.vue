...
<template>
  ...
  <div v-if="store.loading" class="flex flex-col items-center justify-center py-20 text-gray-400">
    <svg class="animate-spin h-8 w-8 mb-3 text-goztepe-red" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
  ...
  <!-- Empty -->
  <div v-else-if="store.filteredGames.length === 0 && store.games.length > 0" class="text-center py-16 text-gray-400 text-sm">
    Seçili liglere ait maç bulunamadı.
  </div>

  <!-- Initial state -->
  <div v-else-if="store.games.length === 0" class="text-center py-16 text-gray-300">
  ...
  <div v-else class="space-y-6">
    <div v-for="(leagueGames, leagueName) in store.gamesByLeague" :key="leagueName">
      <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2 flex items-center gap-2">
        <span class="h-px flex-1 bg-gray-100"></span>
        {{ leagueName }}
        <span class="text-xs font-normal text-gray-400">({{ leagueGames.length }} maç)</span>
        <span class="h-px flex-1 bg-gray-100"></span>
      </h3>
      <div class="space-y-2">
        <GameCard v-for="(game, i) in leagueGames" :key="i" :game="game" />
      </div>
    </div>
  </div>
</template>
