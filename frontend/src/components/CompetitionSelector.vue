  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
    ...
    <div class="flex flex-wrap items-end gap-4">
      ...
      <div class="flex flex-col gap-1">
        <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Sezon</label>
        <select
          :value="store.selectedSeason"
          @change="store.changeSeason(($event.target as HTMLSelectElement).value)"
        ...
        >
          <option v-for="season in store.availableSeasons" :key="season" :value="season">
            {{ season }}
          </option>
        </select>
      </div>
      ...
      <div class="flex flex-col gap-1">
        ...
        <select
          ...
          @change="store.changeCategory(($event.target as HTMLSelectElement).value)"
          class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-goztepe-red focus:border-transparent"
        >
          <option v-for="category in store.availableCategories" :key="category.code" :value="category.code">
            {{ category.name }}
          </option>
        </select>
      </div>
      ...
      <!-- League -->
      <div class="flex flex-col gap-1">
        <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Lig</label>
        <select
          :value="store.selectedLeagueCode"
          @change="store.changeLeague(($event.target as HTMLSelectElement).value)"
          class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-goztepe-red focus:border-transparent"
        >
          <option v-for="league in store.availableLeagues" :key="league.code" :value="league.code">
            {{ league.name }}
          </option>
        </select>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
        ...
        >
          ...
          <div class="text-xs text-gray-500 mt-1">
            ...
            <span v-if="competition.hasGoztepe" class="ml-1 text-goztepe-red">★ Göztepe</span>
          </div>
        </button>
      </div>
    </div>
    ...
    <!-- Loading State -->
    <div v-if="store.loading && store.competitions.length === 0" class="text-center py-4">
      <svg class="animate-spin h-6 w-6 mx-auto mb-2 text-goztepe-red" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <!-- No Competitions -->
    <div v-if="!store.loading && store.competitions.length === 0" class="text-center py-8 text-gray-400">
