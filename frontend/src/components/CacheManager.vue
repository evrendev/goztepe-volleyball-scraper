...
async function loadCacheStatus() {
  ...
  try {
    ...
    caches.value[0].error = e instanceof Error ? e.message : 'Cache durumu alınamadı'
  } finally {
  ...
  try {
    ...
    caches.value[1].error = e instanceof Error ? e.message : 'Cache durumu alınamadı'
  } finally {
...
async function clearCache(type: 'fixtures' | 'standings', seasonId?: string) {
  ...
  try {
    ...
    caches.value[cacheIndex].error = e instanceof Error ? e.message : 'Cache temizlenemedi'
  } finally {
...
<template>
  ...
  >
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 1.79 4 4 4h8c2.21 0 4-1.79 4-4V7M4 7c0-2.21 1.79-4 4-4h8c2.21 0 4 1.79 4 4M4 7h16M16 11l-4 4-4-4" />
    </svg>
  </button>
  ...
  >
    <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-96 overflow-y-auto">
      ...
      <div class="p-4 space-y-6">
        <div v-for="(cache, index) in caches" :key="cache.name" class="space-y-3">
          <div class="flex items-center justify-between">
            <h4 class="font-medium text-gray-900">{{ getCacheDisplayName(cache.name) }} Cache</h4>
          ...
          <!-- Loading -->
          <div v-if="cache.loading" class="flex items-center gap-2 text-sm text-gray-500">
          ...
          <!-- Error -->
          <div v-else-if="cache.error" class="text-sm text-red-600 bg-red-50 p-2 rounded">
            {{ cache.error }}
          </div>
          ...
          <div v-else-if="cache.status" class="space-y-2">
            <div class="text-sm text-gray-600">
              <span class="font-medium">{{ cache.status.totalCachedKeys }}</span> önbellek kaydı
            </div>
            ...
            <div v-if="cache.status.keys.length > 0" class="space-y-1">
              <details class="text-sm">
                ...
                <div class="mt-2 space-y-1 bg-gray-50 p-2 rounded text-xs font-mono max-h-32 overflow-y-auto">
                  <div v-for="key in cache.status.keys" :key="key" class="text-gray-700">
                    {{ key }}
                  </div>
                </div>
              </details>
      ...
      <div class="bg-gray-50 p-4 rounded-b-xl">
        <p class="text-xs text-gray-500">
          Cache temizleme işlemi sonrasında veriler yeniden API'den çekilecektir.
        </p>
      </div>
    </div>
  </div>
